...
public class Client extends GameRenderer {
    ...
    public static AnimatedSprite[] animatedSprites;

    public static AnimatedSprite animatedSpriteForImageId(int id) {
            switch (id) {
                case 836:
    ...
    public static String formatHP(long number){
        if (number < 0x3e8) {
        ...
        if (number < 0xf4240) {
            ...
            return number / 0xf4240 + "M";
        }

    }

    public void hitmarkDrawNew(Entity e, int hitLength, int type, int icon, long damage, long soak, int move, int opacity, int mask) {
        if (spriteDrawX > -1) {
            int drawPos = 0;
            if (mask == 0) {
            ...
            if (damage > 0) {
                ...
                width += end1.myWidth;
                x += 3;
                for (int i = 0; i < hitLength + 3; i++) {
                    middle.drawSprite3(spriteDrawX - 12 + x, drawPos - 12, opacity);
                    width += middle.myWidth;
                    x += 3;
                }
                end2.drawSprite3(spriteDrawX - 12 + x, drawPos - 12, opacity);
                width += end2.myWidth;
                TextDrawingArea text = (type == 1 ? normalText : smallText);
                if (damage < 1000) {
                    text.drawOpacityText(0x5BFF33, formatHP(damage), drawPos + 3, spriteDrawX + (width / 8) - (text.getTextWidth(formatHP(damage)) / 5), opacity);
                }
               else if (damage < 10000) {
                    text.drawOpacityText(0x5BFF33, formatHP(damage), drawPos + 3, spriteDrawX + (width / 8) - (text.getTextWidth(formatHP(damage)) / 5), opacity);
                }
                else if (damage < 100000) {
                    text.drawOpacityText(0x5BFF33, formatHP(damage), drawPos + 3, spriteDrawX + (width / 8) - (text.getTextWidth(formatHP(damage)) / 10), opacity);
                }
                else if (damage < 1000000) {
                    text.drawOpacityText(0x5BFF33, formatHP(damage), drawPos + 3, spriteDrawX + (width / 8) - (text.getTextWidth(formatHP(damage)) / 13), opacity);
                }
                else if (damage < 10000000) {
                    text.drawOpacityText(0xFCFF33, formatHP(damage), drawPos + 3, spriteDrawX + (width / 8) - (text.getTextWidth(formatHP(damage)) / 17), opacity);
                }
                else if (damage < 100000000) {
                    text.drawOpacityText(0xFCFF33, formatHP(damage), drawPos + 3, spriteDrawX + (width / 8) - (text.getTextWidth(formatHP(damage)) / 17), opacity);
                }
                else if (damage > 100000000) {
                    text.drawOpacityText(0xFCFF33, formatHP(damage), drawPos + 3, spriteDrawX + (width / 8) - (text.getTextWidth(formatHP(damage)) / 100), opacity);
                }
                if (soak > 0) {
    ...
    public void drawSoak(long damage, int opacity, int drawPos, int x) {
        x -= 12;
        int soakLength = String.valueOf(damage).length();
        spritesMap.get(793 + 51).drawSprite3(spriteDrawX + x, drawPos - 12, opacity);
        x += 20;
        ...
        smallHit.drawOpacityText(0xffffff, formatHP(damage), drawPos + 32, spriteDrawX - 8 + x + (soakLength == 1 ? 5 : 0), opacity);
    }
    ...
    private void doAction(int actionId) {
        ...
        int fifthMenuAction = menuActionCmd5[actionId];
            ...
            if (childInterface.type == 282) {
                if (childInterface.itemsToDraw != null) {
                    for (int i = 0; i < childInterface.itemsToDraw.length; i++) {
                        ...
                        // well i didnt even add anything  side // numbers
                    }
                        ...
                        for (int l4 = 0; l4 < childInterface.width; l4++) {
                            ...
                            if (i3 < childInterface.inv.length && childInterface.inv[i3] > 0) {
                                ...
                                if (k5 > DrawingArea.topX - 32 && k5 < DrawingArea.bottomX && j6 > DrawingArea.topY - 32 && j6 < DrawingArea.clipBottom || activeInterfaceType != 0 && selectedInventorySlot == i3) {
                                    ...
                                    if (childInterface.id == 30375 && childInterface.inv[i3] == 0) {
    ...
    public void displayLoadingScreen() {
      /*  if (loadingImages[0] == null || loadingImages[1] == null || loadingImages[2] == null || loadingImages[3] == null) {
        ...
        Graphics graphics = canvas == null ? this.getGraphics() : canvas.getGraphics();//this.getGraphics() != null ? this.getGraphics() : canvas.getGraphics();//;
        if (graphics == null) {
    ...
    public void handleHovers(boolean alertScreen) {
        ...
        if (!alertScreen) {
            ...
            if (showTwoFactorAuth) {
                ...
                if (mouseX >= 230 && mouseX <= 270 && mouseY >= 240 && mouseY <= 312) {
                    //cursor = Configuration.NEW_CURSORS ? 1061 : Cursor.HAND_CURSOR;
    ...
    public void drawLoginScreen(boolean flag) {
        ...
        if (Configuration.DISPLAY_GAMEWORLD_ON_LOGIN) {
            ...
            if (titleAlpha < 250) {
                ...
                if (loginHover) {
                    spritesMap.get(464).drawAdvancedSprite(263, 305);//LOGINEDIT
                } else {
                    spritesMap.get(463).drawAdvancedSprite(263, 305);//LOGINEDIT
                }

                if (!showTwoFactorAuth) {

                    if (textArea1Hover) {
                        spritesMap.get(466).drawAdvancedSprite(260, 190);
                    } else {
                        spritesMap.get(465).drawAdvancedSprite(260, 190);
                    }
                    if (textArea2Hover) {
                        spritesMap.get(470).drawAdvancedSprite(260, 238);
                    } else {
                        spritesMap.get(469).drawAdvancedSprite(260, 238);
                    }
                    if (Configuration.SAVE_ACCOUNTS) {
                        spritesMap.get(468).drawAdvancedSprite(250, 288);
                    } else {
                        spritesMap.get(467).drawAdvancedSprite(250, 288);
                    }
                }
                ...
                if (showTwoFactorAuth) {
                    ...
                    if (loginScreenCursorPos == 0 && loopCycle % 45 < 10) {
                        chatTextDrawingArea.drawRegularText(true, 289, 16777215, currentPinCode + "|", 292);
                    } else {
                        chatTextDrawingArea.drawRegularText(true, 289, 16777215, currentPinCode, 292);
                    }
                } else {
                    ...
                    if (loginScreenCursorPos == 0 && loopCycle % 45 < 10) {
                        chatTextDrawingArea.drawRegularText(true, 289, 16777215, myUsername + "|", 213);
                    } else {
                        chatTextDrawingArea.drawRegularText(true, 289, 16777215, myUsername, 213);
                    }

                    if (loginScreenCursorPos == 1 && loopCycle % 45 < 10) {
                        chatTextDrawingArea.drawRegularText(true, 289, 16777215, getStars(password) + "|", 263);
                    } else {
                        chatTextDrawingArea.drawRegularText(true, 289, 16777215, getStars(password), 263);
                    }
                }
    ...
    private void drawSplitPrivateChat() {
        ...
        for (int index = 0; index < 100; index++) {
            if (chatMessages[index] != null) {
                ...
                int color = AnimatedPlayerName.getCurrentColorByRank(rights);
                if(color != 0) {
                    name = "<col=" + color + ">" + name;
                }
...
//        }
                ...
                if (aBoolean1242 && anInt989 >= 15) {
                    ...
                    if (lastActiveInvInterface == modifiedWidgetId && mouseInvInterfaceIndex != selectedInventorySlot) {
                        ...
                        if (class9.dragDeletes) {
                            ...
                            class9.invGlow[l3] = class9.invGlow[l2];
                            class9.inv[l2] = -1;
    ...
    private void readPlayerUpdateMask(int bitmap, int j, ByteBuffer buffer, Player player) {
        ...
        if ((bitmap & 0x20) != 0) {
            long k1 = getInputBuffer().getLong();
            int k2 = buffer.getUnsignedByte();
            int icon = buffer.getUnsignedByte();
            long soakDamage = getInputBuffer().getLong();
            player.updateHitData(k2, k1, loopCycle, icon, soakDamage);
            player.loopCycleStatus = loopCycle + 300;
            player.currentHealth = getInputBuffer().getLong();
            player.maxHealth = getInputBuffer().getLong();
        }
        ...
        if ((bitmap & 0x200) != 0) {
            ...
            int icon = buffer.getUnsignedByte();
            long soakDamage = buffer.getLong();
            player.updateHitData(l2, l1, loopCycle, icon, soakDamage);
            player.loopCycleStatus = loopCycle + 300;
            player.currentHealth = buffer.getLong();
            player.maxHealth = buffer.getLong();
        }
    }
    ...
    private void readNPCUpdateMask(ByteBuffer stream) {
        for (int j = 0; j < playersToUpdateCount; j++) {
            ...
            if ((l & 8) != 0) {
                long j1 = getInputBuffer().getLong();
            ...
            if ((l & 0x40) != 0) {
                long l1 = getInputBuffer().getLong();
            ...
            switch (pktType) {
                ...
                case 125:
                    ...
                    if (readDamageList) {
                        ...
                        for (int t = 0; t < length; t++) {
                            ...
                            long damage = getInputBuffer().getLong(); // Reset again
                ...
                case 53:
                    ...
                    try {
                        ...
                        RSInterface rsi = RSInterface.interfaceCache[rsi_frame];
                        int totalItems = getInputBuffer().getUnsignedShort();
                        if (rsi == null || rsi.inv == null || rsi.invStackSizes == null) {
                            pktType = -1;
                            return true;
                        }
                        int it = -1;
    ...
    void startUp() {
       ...
       // RICH_PRESENCE.initiate();
        ...
        try {
            ...
            animatedSprites[3] = new AnimatedSprite(Signlink.getCacheDirectory() + "gifs/ruby.gif");
            animatedSprites[4] = new AnimatedSprite(Signlink.getCacheDirectory() + "gifs/sapphire.gif");
            animatedSprites[5] = new AnimatedSprite(Signlink.getCacheDirectory() + "gifs/zenyte.gif");
            animatedSprites[6] = new AnimatedSprite(Signlink.getCacheDirectory() + "gifs/mod.gif");
                    ...
                    try {
                        ...
                        if (spriteDrawX > -1) {
                            if (Configuration.NEW_HEALTH_BARS) {
                                ...
                                DrawingArea.drawPixels(5, spriteDrawY - 3, spriteDrawX - 12, 65280, (int) i1);
                                DrawingArea.drawPixels(5, spriteDrawY - 3, (int) (spriteDrawX - 12 + i1), 0xff0000, (int) (30 - i1));
                            }
            ...
            if (ItemStats.itemstats[itemHover] != null) {
                if (ItemStats.itemstats[itemHover].type == 1) {
                    ...
                    for (int i = 0; i < 5; ++i) {
                        ...
                        RSInterface.interfaceCache[94006].message += "@whi@" + bonus + "" + formatNumber(ItemStats.itemstats[itemHover].attackBonus[i]) + "\\n";
                    }
                    ...
                    for (int i = 0; i < 5; ++i) {
                        ...
                        RSInterface.interfaceCache[94007].message += "@whi@" + bonus + "" + formatNumber(ItemStats.itemstats[itemHover].defenceBonus[i]) + "\\n";
                    }
                    RSInterface.interfaceCache[94010].message = "@whi@" + (ItemStats.itemstats[itemHover].strengthBonus > 0 ? "+" : "") + "" + formatNumber(ItemStats.itemstats[itemHover].strengthBonus)
                            + "\\n@whi@" + (ItemStats.itemstats[itemHover].rangeBonus > 0 ? "+" : "") + ""
                            + formatNumber(ItemStats.itemstats[itemHover].rangeBonus) + "" + "\\n@whi@"
                            + (ItemStats.itemstats[itemHover].prayerBonus > 0 ? "+" : "") + "" + formatNumber(ItemStats.itemstats[itemHover].prayerBonus)
                            + "" + "\\n@whi@" + (ItemStats.itemstats[itemHover].magicBonus > 0 ? "+" : "") + "" + formatNumber(ItemStats.itemstats[itemHover].magicBonus) + "";
