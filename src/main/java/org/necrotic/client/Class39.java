...
final class Class39 {
	...
	public void method545(ByteBuffer stream, Class29 class29) {
		...
		if (i != 0) {
			...
			for (int k = 0; k < 2; k++) {
				for (int l = 0; l < anIntArray665[k]; l++) {
					anIntArrayArrayArray666[k][0][l] = stream.getUnsignedShort();
					anIntArrayArrayArray667[k][0][l] = stream.getUnsignedShort();
				}
			...
			for (int i1 = 0; i1 < 2; i1++) {
				for (int j1 = 0; j1 < anIntArray665[i1]; j1++) {
					if ((j & 1 << i1 * 4 << j1) != 0) {
						anIntArrayArrayArray666[i1][1][j1] = stream.getUnsignedShort();
						anIntArrayArrayArray667[i1][1][j1] = stream.getUnsignedShort();
					} else {
