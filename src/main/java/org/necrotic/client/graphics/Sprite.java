...
public class Sprite extends DrawingArea {
	...
	public Sprite(int width, int height, int offsetX, int offsetY, int[] pixels) { // Sprite
		...
		if (!(width == 54 && height == 54) && !(width == 206 && height == 38)) {
			color = Color.WHITE;
			setTransparency(color.getRed(), color.getGreen(), color.getBlue());
		}
	}
...
/*if(ims.contains(img)) {
	...
	public void drawSprite1(int i, int j, int k) {
		...
		if (l1 > 0 && k1 > 0) {
			block_copy_alpha(j1, l1, DrawingArea.raster, myPixels, j2, k1, i2, k, i1);
		}
	}
	...
	public void drawAdvancedSprite(int i, int j, int alpha) {
		...
		if (!(l1 <= 0 || k1 <= 0)) {
			drawAlphaGlow(j1, l1, DrawingArea.raster, myPixels, j2, k1, i2, k, i1);
		}
	}
	...
	public void drawAdvancedSprite(int i, int j) {
		...
		if (!(l1 <= 0 || k1 <= 0)) {
			drawAlphaSprite(j1, l1, DrawingArea.raster, myPixels, j2, k1, i2, k, i1);
		}
	}
	...
	public void drawARGBSprite(int xPos, int yPos, int alpha) {
		...
		if (!(spriteWidth <= 0 || spriteHeight <= 0)) {
			renderARGBPixels(spriteWidth, spriteHeight, myPixels, DrawingArea.raster, i1, alphaValue, j1, j2, i2);
		}
	}
	...
	public void method346(int i, int j) {
		...
		if (k1 <= 0 || j1 <= 0) {
			...
			block_copy(l, k1, j1, i2, i1, l1, myPixels, DrawingArea.raster);
		}
	}
	...
	public void drawSprite3(int x, int y, int opacity) {
		...
		if (!((width <= 0) || (height <= 0))) {
			setPixels(width, height, DrawingArea.raster, myPixels, alpha, destOffset, srcOffset, destStep, srcStep);
		}
	}
	...
	public void drawSprite(int i, int k, int color) {
		...
		if (!(k1 <= 0 || j1 <= 0)) {
			block_copy_trans(DrawingArea.raster, tempArray, i1, l, k1, j1, l1, i2);
		}
	}
	...
	public void draw(Background background, int i, int j) {
		...
		if (!(j1 <= 0 || i1 <= 0)) {
			block_copy_mask(myPixels, j1, background.imgPixels, i1, DrawingArea.raster, 0, k1, k, l1, l);
		}
	}
	...
	public void drawSpriteWithOpacity(int xPos, int yPos, int o) {
		...
		if (!(l1 <= 0 || k1 <= 0)) {
			block_copy_alpha(j1, l1, DrawingArea.raster, myPixels, j2, k1, i2, opacity, i1);
		}
	}
