...
public class RSFontSystem extends DrawingArea {
	...
	public static String handleOldSyntax(String text) {
		...
		//text = text.replaceAll("@bob@", ""+Client.spritesMap.get(105].drawSprite(3, 3); //CacheSprite[497].drawSprite(childX - 3, childY - 3);
					...
					if (effectString.equals(startEffect)) {
						...
						if (effectString.startsWith(startImage)) {
							try {
								...
								AnimatedSprite as;
								if( (as = Client.animatedSpriteForImageId(imageId)) != null) {
									as.getInstance(as.myWidth,
											as.myHeight).drawAdvancedSprite(drawX,
											(drawY + baseCharacterHeight - as.myHeight));
									drawX = drawX + as.myWidth;
								} else {
	...
	private void drawCharacter(int character, int i_35_, int i_36_, int i_37_, int i_38_, int i_39_, boolean bool) {
		...
		if (i_37_ > 0 && i_38_ > 0) {
			try {
				createCharacterPixels(DrawingArea.raster, fontPixels[character], i_39_, i_43_, i_40_, i_37_, i_38_, i_41_, i_42_);
			} catch (Exception ex) {
			}
		}
	}
	...
	private void drawTransparentCharacter(int i, int i_11_, int i_12_, int i_13_, int i_14_, int i_15_, int i_16_, boolean bool) {
		...
		if (i_13_ > 0 && i_14_ > 0) {
			createTransparentCharacterPixels(DrawingArea.raster, fontPixels[i], i_15_, i_20_, i_17_, i_13_, i_14_, i_18_, i_19_, i_16_);
		}
	}
						...
						if (effectString.equals(startEffect)) {
							...
							if (effectString.startsWith(startImage)) {
								try {// <img=
									...
									int iconWidth = Client.spritesMap.get(iconId).myWidth;
