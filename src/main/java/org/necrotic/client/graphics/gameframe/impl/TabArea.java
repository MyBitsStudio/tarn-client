...
public class TabArea extends GameFrame {
			...
			if (client.invOverlayInterfaceID == -1 && Client.tabInterfaceIDs[Client.tabID] != -1) {
				if (GameFrameConstants.gameframeType == GameFrameType.FRAME_525) {
					...
					for (int i = 0; i < Client.tabInterfaceIDs.length; i++) {
						...
						if (i < 8) {
							if (Client.clientWidth > GameFrameConstants.smallTabs && screenMode != ScreenMode.FIXED) {
								offsetX = offsetX - Client.spritesMap.get(527).myWidth / 2 - 10;
								offsetY = offsetY + 297;
							} else if (screenMode != ScreenMode.FIXED) {
								offsetY = offsetY + 261;
								offsetX = offsetX - 9;
						...
						if (Client.tabID == tabID) {
							Client.spritesMap.get(525).drawAdvancedSprite(offsetX, offsetY);
						} else {
							...
							if (Client.tabInterfaceIDs[tabID] != -1 && client.mouseX >= x + (screenMode == ScreenMode.FIXED ? 5 : 5) && client.mouseX <= x + 30 + 4 && client.mouseY >= y && client.mouseY <= y + 34) {
								Client.spritesMap.get(524).drawAdvancedSprite(offsetX + (screenMode == ScreenMode.FIXED ? 5 : 5), offsetY);
							}
						}
					}
				}
	...
	public boolean isHovering(Client client, ScreenMode screenMode) {
		...
		if (GameFrameConstants.gameframeType == GameFrameType.FRAME_525) {
			if (client.inSprite(false, Client.spritesMap.get(7), getOffSetX() + 33, getOffSetY() - (Client.clientWidth <= GameFrameConstants.smallTabs ? 13 : -23)) && !componentHidden()) {
				return true;
			}
			for (int i = 0; i < 14; i++) {
				if (client.inSprite(false, Client.spritesMap.get(screenMode == ScreenMode.FIXED && (i == 0 || i == 7) ? 8 : 12), getxPos() + 6 + (i > 6 ? i - 7 : Client.clientWidth <= GameFrameConstants.smallTabs && screenMode != ScreenMode.FIXED ? i : screenMode == ScreenMode.FIXED ? i : i - 7) * Client.spritesMap.get(screenMode == ScreenMode.FIXED ? i == 1 || i == 8 ? 8 : 12 : 12).myWidth + (screenMode != ScreenMode.FIXED || i == 1 || i == 8 || i == 0 || i == 7 ? 0 : 5), i > 6 || screenMode != ScreenMode.FIXED ? getyPos() + getHeight() - Client.spritesMap.get(12).myHeight * (i < 7 && Client.clientWidth <= GameFrameConstants.smallTabs && screenMode != ScreenMode.FIXED ? 2 : 1) - 1 : getyPos())) {
					return true;
				}
			}
		} else if (GameFrameConstants.gameframeType == GameFrameType.FRAME_554) {
			...
			if (client.clickMode3 == 1) {
				if (GameFrameConstants.gameframeType == GameFrameType.FRAME_525) {
					for (int i = 0; i < 14; i++) {
						...
						if (client.inSprite(true, Client.spritesMap.get(screenMode == ScreenMode.FIXED && (i == 0 || i == 7) ? 8 : 12), getxPos() + 6 + (i > 6 ? i - 7 : Client.clientWidth <= GameFrameConstants.smallTabs && screenMode != ScreenMode.FIXED ? i : screenMode == ScreenMode.FIXED ? i : i - 7) * Client.spritesMap.get(screenMode == ScreenMode.FIXED ? i == 1 || i == 8 ? 8 : 12 : 12).myWidth + (screenMode != ScreenMode.FIXED || i == 1 || i == 8 || i == 0 || i == 7 ? 0 : 5), i > 6 || screenMode != ScreenMode.FIXED ? getyPos() + getHeight() - Client.spritesMap.get(12).myHeight * (i < 7 && Client.clientWidth <= GameFrameConstants.smallTabs && screenMode != ScreenMode.FIXED ? 2 : 1) - 1 : getyPos())) {
							if (screenMode != ScreenMode.FIXED) {
		...
		if (isVisible()) {
			...
			if (screenMode == ScreenMode.FIXED) {
				if (GameFrameConstants.gameframeType == GameFrameType.FRAME_525) {
					...
					Client.spritesMap.get(522).drawSprite(getOffSetX(), getOffSetY());
				}
			} else {
				if (GameFrameConstants.gameframeType == GameFrameType.FRAME_525) {
					for (int i = 0; i < 14; i++) {
						Client.spritesMap.get(6).drawSprite(getOffSetX() + 6 + (i > 6 ? i - (Client.clientWidth <= GameFrameConstants.smallTabs ? 7 : 14) : i) * Client.spritesMap.get(6).myWidth, getOffSetY() + getHeight() - 2 - Client.spritesMap.get(6).myHeight * (i > 6 && Client.clientWidth <= 1000 ? 2 : 1) - 1);
					}
				} else if (GameFrameConstants.gameframeType == GameFrameType.FRAME_554) {
					if (Client.clientWidth <= GameFrameConstants.smallTabs) {
						Client.spritesMap.get(526).drawSprite(getOffSetX() - 2, getOffSetY() + 262);
					} else {
						Client.spritesMap.get(527).drawSprite(getOffSetX() - Client.spritesMap.get(527).myWidth / 2 - 5, getOffSetY() + 298);
					}
				}
			}

			if (!componentHidden() && screenMode != ScreenMode.FIXED) {
				Client.spritesMap.get(18).drawTransparentSprite(getOffSetX() + 39, getOffSetY() + (Client.clientWidth <= GameFrameConstants.smallTabs ? -7 : 29), 100);
	...
	private int[] getSideIconPosForOldFrame(ScreenMode screenMode, Sprite sideIcon, int i) {
		int x = getOffSetX() + (screenMode == ScreenMode.FIXED ? 13 : 8) + (i > 6 ? i - 7 : screenMode != ScreenMode.FIXED && Client.clientWidth > GameFrameConstants.smallTabs ? i - 7 : i) * Client.spritesMap.get(12).myWidth;
